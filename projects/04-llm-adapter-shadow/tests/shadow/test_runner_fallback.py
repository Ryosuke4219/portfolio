...
try:
    ...
    SearchStrategy = Any  # type: ignore[assignment]
else:
...
if HYPOTHESIS_AVAILABLE:
    ...
    def test_provider_request_normalization_boundaries(
        ...
        request = ProviderRequest(
            prompt=prompt_value, messages=messages, model="demo-model"
        )
        ...
        if not expected_prompt and normalized_messages:
            expected_prompt = provider_spi_module._extract_prompt_from_messages(
                normalized_messages
            )
    ...
    def _message_entries() -> Any:  # pragma: no cover - requires hypothesis
        raise RuntimeError("hypothesis is required for strategy generation")


    @pytest.mark.skip(reason="hypothesis is required")
    def test_provider_request_normalization_boundaries(
        prompt: str | None, messages: Sequence[Mapping[str, Any]]
    ) -> None:  # pragma: no cover - requires hypothesis
        pytest.skip("hypothesis is required")
