...
# Hypothesis は任意依存。未導入でも他テストが動くようにする。
try:
...
from _runner_test_helpers import (  # noqa: E402
    FakeLogger,
    _ErrorProvider,
    _SuccessProvider,
    _SkipProvider,
...
def test_rate_limit_triggers_backoff_and_logs(
    ...
    # sync ランナー側の backoff sleep をフック
...
def test_async_rate_limit_triggers_backoff_and_logs(
    ...
    # async ランナー側の backoff sleep をフック
...
# ---- Hypothesis-based property tests (存在時のみ定義) ----
if _HAS_HYPOTHESIS:
