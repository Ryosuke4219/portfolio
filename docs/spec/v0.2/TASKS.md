...
- 残タスク:
  - CLI リテラル API キー経路: CLI/設定ファイルから `ProviderRequest.options["api_key"]` へ値を渡す経路が未実装。`adapter/cli/app.py` のマッピングと CLI テストを整備し、OpenRouter との切替でもキーが取り違わないことを確認する。
...
- 対応状況:
  ...
  - CLI からリテラル API キーを渡す経路は未接続で、CLI 入力が `ProviderRequest.options["api_key"]` へマッピングされないギャップが残っている。
- 品質エビデンス:
...
- 残タスク:
  - CLI リテラル API キー経路: CLI/設定ファイルから `OPENROUTER_API_KEY` を受け取り、`ProviderRequest.options["api_key"]` へ結線するマッピングを実装し、`projects/04-llm-adapter/tests/test_cli_single_prompt.py::test_cli_auth_env_accepts_literal_api_key` で回帰テストを担保する。
