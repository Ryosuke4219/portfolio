...
<div class="demo-grid">
  ...
  <article class="demo-card" id="demo-04">
    ...
    <ul>
      ...
      <li><code>llm-adapter --provider adapter/config/providers/openai.yaml --prompt "日本語で1行、自己紹介して"</code> で単発実行し、<code>python adapter/run_compare.py</code> が比較ランを一括管理。</li>
...
- `llm-adapter --provider adapter/config/providers/openai.yaml --prompt "日本語で1行、自己紹介して"` で単発実行し、`python adapter/run_compare.py` が比較ランを `data/runs-metrics.jsonl` に記録。
